using Microsoft.Maui.Controls.PlatformConfiguration;

namespace ShopPlus.Pages;

public partial class LoginPage : ContentPage
{
	public LoginPage()
	{
		InitializeComponent();
    }

    private async void btnEntrar_Clicked(object sender, EventArgs e)
    {
        string email = txtEmail.Text;
        string senha = txtSenha.Text;

        if (!string.IsNullOrWhiteSpace(email) && !string.IsNullOrWhiteSpace(senha))
        {
            var usuario = await App.BancoDados.UsuarioDataTable.ObtemUsuario(email, senha);

            if (usuario != null)
            {
                await DisplayAlert("Sucesso", "Login efetuado com sucesso", "OK");
                await Navigation.PushAsync(new HomePage());
                App.Usuario = usuario;
            }
            else
            {
                await DisplayAlert("Erro", "Usuário ou senha inválidos", "OK");
                return;
            }
        }
        else
        {
            if (string.IsNullOrWhiteSpace(email))
            {
                await DisplayAlert("Erro", "Preencha o campo de e-mail", "OK");
            }
            else if (string.IsNullOrWhiteSpace(senha))
            {
                await DisplayAlert("Erro", "Preencha o campo de senha", "OK");
            }
        }
    }
        private void OnCheckBoxCheckedChanged(object sender, CheckedChangedEventArgs e)
    {
        bool lembrarLogin = e.Value;
        Preferences.Set("LembrarLogin", lembrarLogin); // Salva o estado do CheckBox nas preferências compartilhadas
    }

    protected override void OnAppearing()
    {
        base.OnAppearing();

        // Recupera o estado do CheckBox ao abrir a página
        bool lembrarLogin = Preferences.Get("LembrarLogin", false);
        chkLembrarLogin.IsChecked = lembrarLogin;
    }

    private async void btnRegistrar_Clicked(object sender, EventArgs e)
    {
        await Navigation.PushAsync(new CadastroPage());
    }

    private async void OnCadastreSeClicked(object sender, EventArgs e)
    {
        // Navegar para a página de cadastro
        await Navigation.PushAsync(new CadastroPage());
    }

    private async void OnShopPlusLabelTapped(object sender, EventArgs e)
    {
        await Navigation.PushAsync(new HomePage());
    }

    protected override void OnHandlerChanged()
    {
        base.OnHandlerChanged();
#if ANDROID
    var edittext = txtEmail.Handler.PlatformView as Android.Widget.EditText;
    edittext.Background = null;
#endif
    }

}